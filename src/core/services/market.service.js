...
class MatketService {
  ...
  getHashtagNames(name, limit) {
    return http.get(`hashtags/?${apiQueryBuilder({ name, limit })}`);
  }
  ...
  unHideToken(userId, chain, tokenAddress, tokenId) {
    return http.patch(
      `users/${userId}/token/unhide?chain=${chain}&tokenAddress=${tokenAddress}&tokenId=${tokenId}`
    );
  }
  ...
  bundledTokens(chain, userId) {
    return http.get(`users/tokens/bundled?chain=${chain}&userId=${userId}`);
  }
  ...
  getContracts(data) {
    return http.get(`contracts/search?${apiQueryBuilder(data)}`);
  }
  ...
  getContract(chain, address) {
    return http.get(`contracts/${chain}/${address}`);
  }
  ...
  searchNfts(data) {
    return http.get(`nfts/search?${apiQueryBuilder(data)}`);
  }
