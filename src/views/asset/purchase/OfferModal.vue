...
import { useEthers, displayEther, useWallet } from "vue-dapp";
...
const makeOffer = async () => {
  ...
  if (isNaN(offerPrice.value) || offerPrice.value <= 0) {
    toast.error("Offer price can't be zero");
    return;
  }
  if (walletBalanceInEth.value < offerPrice.value) {
  ...
  try {
    ...
    if (props.asset.mlrContract) {
      createOfferInMLR(
        ...
        Web3.utils.toWei(offerPrice.value.toString(), "ether"),
      ...
      createOffer([
        ...
        tokenId,
        selectedWallet.value === "Vault Balance"
          ? store.state.user.bscTestnetVault.address
          : NULL_ADDRESS,
        Web3.utils.toWei(offerPrice.value.toString(), "ether"),
        staking.value,
        (100 - slippage.value) * 100,
  ...
  >
    <div class="bg-tertiary-900 text-white xl:w-modal-md">
      ...
      <div class="text-center relative mt-3">
        ...
        <div
          class="font-ibm text-primary-900 text-sm sm:text-lg mt-4 flex justify-center"
        >
          Gain APY rewards while your offer is pending
          <nftmx-tooltip
            class="flex"
      ...
      <div class="text-white px-4 lg:px-17.25 pb-12">
        <div class="relative text-lg font-ibm-medium">
          ...
          <div class="sm:flex mt-3.25 font-ibm text-sm">
            <div
              :class="[
                warn && !offerPrice ? 'border-red-900' : 'border-black',
            ...
            >
              ...
              >
                ${{
                  offerPrice && coinPrice
                    ? (offerPrice * coinPrice).toFixed(2)
                    : 0
                }}
              </div>
            </div>
            ...
            <div>
              ...
              >
                <div class="flex items-center gap-4">
                  <nftmx-radio :active="staking" @click="staking = true" />
                  <span>Yes</span>
                </div>
                <div class="flex items-center gap-4">
                  <nftmx-radio :active="!staking" @click="staking = false" />
                  <span>No</span>
                </div>
              </div>
            </div>
          ...
          <div
            ...
            class="text-center font-ibm-medium text-11 text-primary-900 cursor-pointer h-0 relative -top-5"
            @click="openVaultModal = true"
          >
            Add fund to your vault
          </div>
          <div
            v-if="
              selectedWallet === 'Wallet Balance' &&
              walletBalanceInEth < offerPrice
            "
