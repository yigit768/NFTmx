...
const checkFollowing = () => {
  ...
  usersService
    .checkFollowing(walletAddress.value, accountAddress)
    .then((res) => {
      iFollowing.value = res.data;
    });
};
onMounted(() => {
  retrieveFollowers();
...
const follow = () => {
  ...
  loading.value = true;
  usersService
    .follow(walletAddress.value, accountAddress)
    .then((res) => {
...
const unfollow = () => {
  ...
  loading.value = true;
  usersService
    .unfollow(walletAddress.value, accountAddress)
    .then((res) => {
      followers.value.count--;
...
<template>
  <nftmx-summary-container :img_url="profileBanner">
    <div class="flex">
      ...
      >
        ...
        <div class="mt-2.5">
          ...
          >
            {{ following.count.toLocaleString() }}
          </span>
        </div>
    ...
    <followers-modal v-model="followerModal" :users="followers.users" />
    <following-modal v-model="followingModal" :users="following.users" />
