  ...
  try {
    ...
    if (!derivativeContractAddress.value) {
      ...
      derivativeContractAddress.value = (
        await createDerivative(props.asset.tokenAddress, props.asset.tokenId, 0)
...
const getApprovedStatus = async () => {
  if (chainId.value !== 97 || !props.asset || !props.asset.tokenAddress) return;
      ...
      <div v-if="asset && asset.contract" class="px-4 lg:px-18 pb-18 pt-7.5">
        ...
        <div v-if="step === 0" class="flex flex-col items-center">
          ...
          <div class="w-fit">
            <nftmx-img
              :src="asset.image512"
              class="w-[480px] h-90.75 border border-black mt-12.75 object-center object-cover bg-black"
            />
            ...
            <div class="flex justify-between text-2xl mt-1.5">
              ...
              <div class="flex items-center justify-center text-sm gap-3">
                <div v-if="asset.chain === 'BSC-TESTNET'" class="w-4 h-4">
                  <svg
                    role="img"
                    viewBox="0 0 24 24"
